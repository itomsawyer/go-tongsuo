...
import (
	...
	"github.com/itomsawyer/go-tongsuo"
)
...
type SM3Hash struct {
	ctx    *C.EVP_MD_CTX
	engine *tongsuo.Engine
}

func New() (*SM3Hash, error) { return NewWithEngine(nil) }

func NewWithEngine(e *tongsuo.Engine) (*SM3Hash, error) {
...
func (s *SM3Hash) Reset() {
	C.EVP_DigestInit_ex(s.ctx, C.EVP_sm3(), (*C.ENGINE)(s.engine.CPointer()))
}
...
func (s *SM3Hash) Write(p []byte) (n int, err error) {
	...
	if 1 != C.EVP_DigestUpdate(s.ctx, unsafe.Pointer(&p[0]), C.size_t(len(p))) {
